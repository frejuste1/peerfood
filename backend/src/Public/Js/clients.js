document.addEventListener('DOMContentLoaded', function () {
    ...
    const API_ENDPOINTS = {
        ...
        GET: (id) => `http://localhost:8086/customer/${id}`,
        ...
        BULK_DELETE: 'http://localhost:8086/customer/bulk-delete',
        BULK_STATUS: 'http://localhost:8086/customer/bulk-status'
    };
    ...
    function initFormSteps() {
        ...
        if (!nextBtn) {
            console.error("L'élément #nextStep n'a pas été trouvé dans le DOM.");
            return;
        }
        if (!prevBtn) {
            console.error("L'élément #prevBtn n'a pas été trouvé dans le DOM.");
            return;
        }
        if (!saveClientBtn) {
            console.error("L'élément #saveClient n'a pas été trouvé dans le DOM.");
            return;
        }
        if (!addClientBtn) {
            console.error("L'élément [data-action='add-client'] n'a pas été trouvé dans le DOM.");
            return;
        }
        if (!modal) {
            console.error("L'élément #newClientModal n'a pas été trouvé dans le DOM.");
            return;
        }
        ...
        saveClientBtn.addEventListener('click', (event) => {
            if (validateCurrentStep()) {
    ...
    function initApp() {
        if (!modal) {
            console.warn("Le modal #newClientModal n'existe pas sur cette page. Initialisation partielle.");
    ...
    customerId.addEventListener("input", () => {
        ...
        if (!regex.test(customerId.value)) {
            customerIdHelp.textContent = "⚠️ Format invalide (6-10 caractères, lettres majuscules et chiffres)";
    ...
    function validateCurrentStep() {
        ...
        if (currentStep === 1) {
            ...
            if (!clientRole.checkValidity() || !['Student', 'Teacher'].includes(clientRole.value)) {
    ...
    async function updateClient(event) {
        ...
        try {
            ...
            if (clientPassword.value.trim()) {
                formData.password = clientPassword.value.trim();
            }
    ...
    async function fetchClients(page = 1, status = "") {
        ...
        if (searchTerm) {
            apiUrlWithParams += `&search=${encodeURIComponent(searchTerm)}`;
        }
        ...
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="6" class="text-center">Erreur lors du chargement des données</td></tr>`;
        } finally {
            hideLoader();
        }
    }
    ...
    function renderClients(clients) {
        ...
        clients.forEach(client => {
            const status = client.statut === 'Enabled' ? 'success' : 'warning';
            const isChecked = client.statut === 'Enabled' ? 'checked' : '';
            const row = document.createElement('tr');
            row.innerHTML = `
                <td><input type="checkbox" class="form-check-input" value="${client.id}"></td>
                <td>
                ...
                <td>
                    <div class="status-container d-flex align-items-center gap-2">
                        <span class="status-badge badge ${status}">${client.statut || 'Inconnu'}</span>
    ...
    async function handleBulkAction(e) {
        ...
        switch (action) {
            case 'bulk-delete':
                await confirmBulkDelete(selectedRows);
                break;
            case 'bulk-export':
    ...
    async function confirmBulkDelete(rows) {
        ...
        const selectedIds = rows.map(row => row.querySelector('.form-check-input').value);

        showLoader();

        try {
            const response = await fetch(API_ENDPOINTS.BULK_DELETE, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
            ...
            showNotification('Clients supprimés avec succès', 'success');
            fetchClients();
            const bulkActions = document.querySelector('.bulk-actions');
            if (bulkActions) {
                bulkActions.classList.remove('show');
    ...
    async function handleStatusToggle(event) {
        if (event.target.classList.contains('status-toggle')) {
            ...
            const clientId = row.querySelector('.form-check-input').value;
    ...
    function initSearchAndFilters() {
        ...
        statusFilter.addEventListener('change', () => {
            currentPage = 1;
            fetchClients(currentPage, statusFilter.value);
        });
    ...
    async function editClient(row) {
        const clientId = row.querySelector('.client-id').textContent.replace('#CLD', '');
        showLoader();

        try {
            const response = await fetch(API_ENDPOINTS.GET(clientId), {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            });
            ...
            customerId.value = client.customerId || clientId;
            clientUsername.value = client.username || '';
            clientPassword.value = '';
            clientRole.value = client.role || '';

            modal.style.display = 'HABILLAGE';
            modal.classList.add('show');
            document.body.classList.add('modal-open');
            ...
            saveClientBtn.textContent = 'Mettre à jour';
            goToStep(1);
        } catch (error) {
